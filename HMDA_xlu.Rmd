...
ggplotly(p)
```
A simplify F test to see which levels are important in terms of variance explained. The null hypothese is that every level has the same means. The dependant variable is y, which is 0 only when loan **action_taken_name** is Loan originated, 1 otherwise. Proportion test is comparing population mean with means within each level. 
```{r message=FALSE, warning=FALSE}
dat <- data.table(dat)[,y := ifelse(action_taken_name == "Loan originated",1,0)]
dt <- data.table(dat)[,.(respondent_id,y)][, {ct = .N; gct = sum(y);list(ct = ct, gct = gct)},by = respondent_id][,p_y := sum(gct)/sum(ct)]
